...
以下の定義は，\cite[Section 3]{MR0764184}で定義されたものと同一であるが，本質的には\cite[Theorem 4]{MR0403962}で定義されたものである．

まず，$f \in R[X_1, \dots, X_n]$に対し，$\red(f):=f - \LT(f; X_n)$と定義する．
ただし，$f=0$の場合，$\red(f)=0$とする．さらに，
\begin{align}
     \red^0(f)&:=f\\
     \red^{n+1}(f)&:=\red(\red^n(f))
\end{align}
により，非負整数$n$に対して帰納的に$\red^n(f)$を定義する．また，
\begin{equation}
     \RED(f) := \{ \mathrm{red}^k(f) \mid f \in F, k=0, \dots, \deg(f; X_n) \} \setminus \{0\}
\end{equation}
とする．$f=0$の場合，$\RED(f) = \emptyset$となることに注意しておく．

次に，$f, g \in R[X_1, \dots, X_n] \setminus \{0\}$に対して，
\begin{equation}
     \PSC(f,g) := \{\psc_j(f,g) \mid j = 0, \dots, \min\{\deg(f;X_n), \deg(g;X_n)\}\} \setminus \{0\}
\end{equation}
と定義する．また，$f, g$のいずれかが$0$である場合には，$\PSC(f,g):=\emptyset$と定義する．

\begin{definition}
$F \subset R[X_1, \dots, X_n]$を有限部分集合とする．ただし，$n \geq 2$とする．
\begin{align}
     \PROJ_1(F) &:= \bigcup_{f \in F} \bigcup_{A \in \RED(f)} \left(\{\LC(A;X_n)\} \cup \PSC\left(A, \frac{\partial A}{\partial X_n}\right)\right)\\
     \PROJ_2(F) &:= \bigcup_{f \neq g \in F} \bigcup_{A \in \RED(f), B \in \RED(g)} \PSC(A,B)
\end{align}
とし，以上を用いて
\begin{align}
	\PROJ(F) &:= \PROJ_1(F) \cup \PROJ_2(F)
\end{align}
と定める．
\end{definition}
