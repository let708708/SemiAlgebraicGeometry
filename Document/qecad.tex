...
\todo[inline]{
     実体，順序体の定義，およびその同値性は，初出の論文が分からない．
}
まず，\cite{MR3069467}でArtinとSchreierにより導入された，実閉体の概念についてまとめる．
実閉体は，実数体を拡張して定義される概念であり，実閉体上でも実数体と類似した解析的な性質が成り立つ．

まず始めに，順序体の定義を述べる．以下の定義は，\cite[Definition 1.1.1.]{MR1659509}に従った．

\begin{definition}[順序体の定義]
...
\cite[Theorem 1.1.8.]{MR1659509}によると，以下の定理が成り立つ．

\begin{theorem}\label{theorem:real-field}
     $F$を体とする．このとき，以下は同値である．
     \begin{enumerate}
          \item \label{theorem:real-field-1}
          $F$が順序体となるような全順序構造$<$を持つ．
          \item \label{theorem:real-field-2}
          各$x_1, \dots, x_n \in F$について，
          \begin{equation}
               x_1^2 + \dots + x_n^2 \neq -1.
          \end{equation}
...
\cref{theorem:real-field}の証明は，\cite[Theorem 1.1.8.]{MR1659509}を参照してほしい．

さらに，\cite[Definition 1.1.9.]{MR1659509}に従い，実体を定義する．

\begin{definition}[実体の定義]
     体$F$が，\cref{theorem:real-field}の
     \ref{theorem:real-field-1}，\ref{theorem:real-field-2}，\ref{theorem:real-field-3}いずれかの条件を満たすとき，
     体$F$を実体(real field)という．
\end{definition}
...
また，複素数体$\C$は$\sqrt{-1}^2 = -1$をみたす．したがって，$\C$は実体ではない．
...
\begin{theorem} \label{theorem:real-closed-field}
     体$F$について，以下は同値である．
     \begin{enumerate}
          \item 体$F$は実閉体である．
...
\cref{theorem:real-closed-field}の証明は，\cite[Theorem 1.2.2.]{MR1659509}を参照してほしい．
...
実閉体$R$を係数とする$1$変数多項式について，実数$\R$上の連続関数，可微分関数と同様の性質が成り立つ．
...
\begin{proof}
     ...
     \end{equation}
     とする．このとき，$(a,b)$上$g(X)$の根がないことから，\cref{proposition:intermediate}より，$g(a)g(b)>0$である．
...
\begin{corollary}\label{corollary:monotone}
     $R$を実閉体とし，$f \in R[X]$とする．また，$a, b \in R$を$a<b$とする．このとき，以下が成り立つ．
     \begin{enumerate}
          \item \label{corollary:monotone-1}
          任意の$x \in (a,b)$について$f'(x)>0$ならば，$f(X)$は$[a,b]$上狭義単調増加である．
          \item \label{corollary:monotone-2}
          任意の$x \in (a,b)$について$f'(x)<0$ならば，$f(X)$は$[a,b]$上狭義単調減少である．
     \end{enumerate}
\end{corollary}
\begin{proof}
     ...
     この$x \in (a,b)$に対して$f'(x) \leq 0$となり，\ref{corollary:monotone-1}の証明が完了した．
\end{proof}
