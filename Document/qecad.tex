...
$(x_1,\dots,x_N)$と置く．
$m:= \max\{\deg(f_i) \mid i=1, \dots, s\}$と置くと，$N \leq sm$である．
また，$x_0 := -\infty$,
$x_{N+1} := \infty$とし，
各$k=0, \dots, N$に対し$I_k := (x_k, x_{k+1})$と書く．
...
\begin{proof}
     \begin{equation}
          W(\sigma):=\{A \in W_{s,m} \mid \text{行列$A$のある列が$(\sigma(i))_{i=1}^s$である}\}
     \end{equation}
     とすればよい．
\end{proof}
...
\begin{proof}
     ...
     $
     のみで決まる．
     $i_0:=0$, $i_{M+1}:=N+1$, $x_0:= -\infty$, $x_{N+1}:= \infty$と置く．
