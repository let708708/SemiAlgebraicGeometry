...
\begin{lemma}\label{lemma:del_1}
     $S \subset R^{n-1}$を半代数的連結な半代数的集合部分集合とし，
...
\begin{lemma}\label{lemma:del_2}
     ...
     さらに，次を満たすとする．
     \begin{itemize}
          \item $S$上$f$の$R[\sqrt{-1}]$上の根の数は重複度込みで一定．
          \item $S$上$f$の$R[\sqrt{-1}]$上の相異なる根の数は一定．
     \end{itemize}
     このとき，$S$は$\{f\}$描画可能である．
\end{lemma}
...
\begin{proof}
     ...
     $\map{N}{S}{R}$を，各$a \in F$に対し，$N(a)$を$f(a,X_n)$の相異なる$R$上の根の数で定める，
     すると，$N$は半代数的関数である．\footnote{これも同じように1階述語論理式出かけることから従う．}
     
     さらに，$N$が局所定数関数であることを示す．
     $a \in S$を任意にとり，$f(a,X_n)$の$R$上の根を$z_1, \dots, z_s$, それ以外の根を$z_{s+1}, \dots, z_t$とする．
     $R$の元$r > 0$を，$\{D(z_i,r)\}_{i=1}^t$が非交叉になるようにとると，\cref{proposition:continuity-of-roots}より，
     ある$a$の開近傍$V$が存在し，各$x \in V \cap S$に対して$f (x, X_n)$ の根が各 $D(z_i, r)$ ($i = 1, \dots , t$) の中に（重複
     度込みで）ちょうど $\mu_i$ 個であるようにとれる．
     ここで，仮定より$S$の相異なる根の数は一定であるから，$\{D(z_i, r)\}_{i=1}^t$は非交叉であること合わせて，
     ...
     よって，$S$が半代数的連結であることと合わせて，$N$は定数関数である，すなわち，相異なる$R$上の根の個数は$S$上一定である．
     ...
     さらに，各$\map{\xi_i}{S}{R}$は，各グラフが半代数的集合となることからいずれも半代数的関数である．
...
よって，$S$上の連続な半代数的関数$\alpha_{1,f}(a) < \dots \alpha_{n_f, f}(a)$を，各$a \in S$で$f(a)(x) \in \R[x]$の解であるようにとれる．
