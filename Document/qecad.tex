...
\begin{remark}
     ...
     \begin{itemize}
          ...
          \item
               有限個の論理式$\phi_1, \dots, \phi_n$に対して，$\bigvee_{i=1}^n \phi_i$を$\phi_1 \lor \dots \lor \phi_n$\todo{括弧記号}の省略とする．
...
\begin{example}[実閉体の理論]
     言語$\mathcal{L}_\mathrm{OR}$を以下で\todo{助詞の検討}定める．
     \begin{itemize}
          \item 定数記号 $0$, $1$.
          \item 1変数関数記号 $-$.
          \item 2変数関数記号 $+$, $\cdot$.
          \item 2変数関係記号 $<$.
     \end{itemize}
     このとき，言語$\mathcal{L}_\mathrm{OR}$上の実閉体の理論$\RCF$を，次の$\mathcal{L}_\mathrm{OR}$文からなる集合で\todo{助詞の検討}定義する．
...
\cref{lemma:qe_simplify}により，\cref{theorem:Tarski}は，次のように言い換えることができる．
...
\begin{proposition} \label{proposition:weak_Tarski}
...
\begin{proof}[\cref{proposition:weak_Tarski}の証明]
     ...
     \end{itemize}
     ここで，$g_1, \dots, g_s$は，それぞれ$f_s$を$f_1, \dots, f_{s-1}$, $f_s'$を$X$に関して割った余りに，
     $h_{1, m_1}, \dots, h_{s, m_s}$の適切な偶数べき乗を，それぞれの分母が消去できるようにかけて得られる多項式である．
