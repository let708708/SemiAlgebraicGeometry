...
\begin{proposition}
     $\phi_1, \dots \phi_n$を$\mathcal{L}$論理式とし，$\psi$を$\phi_1, \dots, \phi_n$のBoolean combination\todo{未定義}とする．
     このとき，$\{1, \dots, n\}$の部分集合族$S$が存在し，
     \begin{equation}
          \models \psi \leftrightarrow \bigvee_{X \in S}\left( \bigwedge_{i \in X} \phi_i \wedge \bigwedge_{i \not \in X} \lnot \phi_i \right)
     \end{equation}
     を満たす．
\end{proposition}
...
\begin{proposition}
     ...
     \begin{equation}
          \models \phi(w_1, \dots, w_n) \leftrightarrow \Qua_1 v_1 \dots \Qua_m v_m \psi(v_1, \dots, v_m, w_1, \dots, w_n)
     \end{equation}
