...
\section{導入}\label{section:1}
     ...
     また，$\mathcal{R}$を一意分解整域とし，$\mathcal{R}$係数の多項式$f \in R[X]$に対し，
     次数を$\deg(f;X)$, 先頭項を$\LT(f;X)$, 先頭項係数を$LC(f;X)$で表す．
     ただし，変数$X$が明らかである場合は，$\deg(f)$, $\LT(f)$, $LC(f)$の書く．
     最大公約元を$\gcd(f(X), g(X);X)$で表す．
     また，$f, g \in \mathcal{R}[X]$に対し，
     $f$, $g$の最大公約元を$\gcd(f,g;X)$で表す．
     同様に，変数$X$が明らかである場合は$\gcd(f,g)$と書く．
...
$R\left[ \sqrt{-1} \right]$は，$R^2$と同一視して位相が定まることに注意する．
