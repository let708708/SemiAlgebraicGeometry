...
\begin{proof}
     \cref{proposition:dnf}より，有限個の$F_{i,j} \in \Z\left[\overline{X}, \overline{T}\right]$および$\sigma_{i,j} \in \{-1, 0 ,1\}$, $i=1, \dots, s, j = 1, \dots, l_i$, が存在して，
     \begin{equation}
          R \models
...
\end{proof}
各多項式$f_{i,j}\left(\overline{X}\right) \in R\left[\overline{X}\right]$について，
各項の係数も変数とみなせば，ある$F_{i,j}\left(\overline{X}, \overline{T}\right) \in \Z\left[\overline{X}, \overline{T}\right]$及び$\overline{t} \in R^k$が存在して
$f_{i,j}\left(\overline{X}\right) = F_{i,j}\left(\overline{X}, \overline{t}\right)$を満たす．このとき，
\begin{equation}
     S = \left\{ \overline{x} \in R^n \middle| R \models \bigvee_{i=1}^s \bigwedge_{j=1}^{l_i} \sign(F_{i,j}(\overline{x}, \overline{t})) = \sigma_{i,j} \right\}
\end{equation}
である．
...
\begin{proposition} \label{proposition: Seidenberg-Tarski-generalization}
     $\phi(X_1, \dots, X_n, Y_1, \dots, Y_m)$を，$\mathcal{L}_\mathrm{OR}$論理式とする．また，$\overline{y} = (y_1, \dots, y_m) \in R^m$とする．
     このとき，
     \begin{equation}
          \left\{\overline{x} \in R^n \middle| R \models \phi(\overline{x},\overline{y}) \right\}
     \end{equation}
     により定まる集合は，$R^n$の半代数的部分集合である．
...
\todo[inline]{以下の命題は条件(2)が「相異なる実根」と，少し違った形で述べられている．誤植だと思う．}

\begin{proposition}[{\cite[Proposition 5.14.]{MR2248869}}]\label{proposition:del}
     $F \subset R[X_1, \dots, X_n]$を有限部分集合とし，$S \subset R^{n-1}$を半代数的連結な半代数的集合とする．
     さらに，以下の条件を満たすとする．
     \begin{enumerate}
...
\begin{proof}
     ...
     また，関数$\xi_i$が半代数的であることは，$\xi_i$のグラフが1階述語論理式で書けることから従う．\todo{説明が必要そう}
...
\subsection{部分終結式係数}
\begin{definition}[{\cite[Notation 4.21.]{MR2248869}}]
$\mathcal{R}$を可換環とし，$P(X), Q(X) \in \mathcal{R}[X] \setminus \{0\}$ を，$\deg P(X) = p$, $\deg Q(X) = q$ ( $p > q$ ) とする．
また，$P(X)$, $Q(X)$は次のように書けるとする．
\begin{align}
     P(X) = a_p X^p + \dots + a_1 X + a_0, \\
     Q(X) = b_q X^q + \dots + b_1 X + b_0.
\end{align}
このとき，$0 \leq j \leq q$に対し，$j$次Sylvester-Habicht行列$\SyHa_j(P, Q)$を，
\begin{equation}
     \SyHa_j(P, Q) \coloneqq
     \begin{pmatrix}
          ...
          0      & \cdots & \cdots & 0      & b_q    & \cdots & \cdots & \cdots & b_0    \\
          \vdots &        & \iddots& \iddots&        &        &        & \iddots& 0      \\
          \vdots & \iddots& \iddots&        &        &        & \iddots& \iddots& \vdots \\
          0      & b_q    & \cdots & \cdots & \cdots & b_0    & 0      & \cdots & 0      \\
          b_q    & \cdots & \cdots & \cdots & b_0    & 0      & 0      & \cdots & 0      
     \end{pmatrix}
\end{equation}
で定義する．
$\SyHa_j(P,Q)$は$\mathcal{R}$係数$(p+q-2j)$行$(p+q-j)$列行列であり，
各行は多項式
\begin{equation}
     X^{q-j-1}P(X), \dots, P(X), Q(X), \dots, X^{p-j-1}Q(X)
\end{equation}
の$X^{p+q-j-1}, \dots, X, 1$の係数を順に並べたものである．

また，$\SyHa_{j,j}(P,Q)$を，$\SyHa_j(P,Q)$の初めの$p+q-2j$列により得られる$(p+q-2j)$行$(p+q-2j)$列行列とし，
$j$次符号付き部分終結式係数($j$-th subresultant coefficient) $\sRes_j(P,Q)$ を
\begin{equation}
     \sRes_j(P,Q) \coloneqq \det(\SyHa_{j,j}(P,Q))
\end{equation}
と定義する．

また，$q < j \leq p$に対しても，
\begin{align}
     \sRes_p(P,Q) &\coloneqq \sign(a_p),\\
     \sRes_j(P,Q) &\coloneqq 0, \quad q < j < p
\end{align}
と，$\sRes_j(P,Q)$の定義を拡張しておく．
\end{definition}

主部分終結式係数は，多項式$P(X)$, $Q(X)$の係数，及び次数に依存して定まることに注意する．

このとき，次が成り立つ．

\begin{theorem}[{\cite[Proposition 4.25.]{MR2248869}}] \label{theorem:psc} \todo{証明確認}
     $\mathcal{R}$を整域とし，$P(X), Q(X) \in \mathcal{R}[X] \setminus \{0\}$を，$\deg P(X) = p$, $\deg Q(X) = q$ ( $p>q$ ) とする．
     このとき，各$j = 0, \dots, q$に対し，次の2つが同値になる．
     \begin{enumerate}
          \item $\deg(\gcd(P,Q)) = j$である．
          \item $\sRes_0(P,Q) = \cdots = \sRes_{j-1}(P,Q) = 0$, $\sRes_j(P,Q) \neq 0$である．
     \end{enumerate}
\end{theorem}

証明は，\cite[Proposition 4.25.]{MR2248869}を参照してほしい．

\subsection{符号不変な柱状代数分解の存在}\todo{ここから}
...
以下の定義は，\cite[Notation 5.15.]{MR2248869}
...
\begin{align} 
     &\{\LC(A;X_n) \mid A \in \Tru(F;X_n) \} \setminus R,\\
     &\left\{ \sRes_j\left(A,\frac{\partial A}{\partial X_n};X_n\right) \middle| A \in \Tru(F;X_n), j= 0, \dots, \deg(A;X_n)-2\right\} \setminus R,\\
     &\{\sRes_j(A,B;X_n) \mid A ,B \in \Tru(F; X_n), \deg(A;X_n) > \deg(B;X_n),   j = 0, \dots, \deg(B;X_n)\} \setminus R,\\
     &\{\sRes_j(A,\overline{B};X_n) \mid A ,B \in \Tru(F; X_n), \deg(A;X_n) = \deg(B;X_n),   j = 0, \dots, \deg(B;X_n)\} \setminus R,
\end{align}
ただし，$\overline{B} \coloneqq LC(A;X_n) B - LC(B;X_n) A$とする．
\end{definition}
