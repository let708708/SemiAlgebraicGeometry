...
\section{導入}\label{section:1}
     ...
     しかし，Tarskiの量化記号消去のアルゴリズムの実行時間は，指数関数の有限回の合成で抑えることはできないことが知られている．
     ...
     実数についての任意の条件\todo{推敲}は，量化記号消去ができるということは，
     ...
     を満たすようにとることができ，このとき
     \begin{align}
          C_{S_i, 2j} &= \{(a,\xi_{S_i, j}(a)) \in S_i \times \R \mid a \in S_i\}, \quad j=1,...,l_i,\\
          C_{S_i, 2j+1} &= \{(a,x) \in \R^2 \mid a \in S_i, \xi_{S_i,j}(a) < x < \xi_{S_i, j+1}(a)\}, \quad j=0,...,l_i,
     \end{align}
     と定めると，$\{C_{S_i,k}\}_{k=1}^{2l_i+1}$は$S_i \times \mathbb{R}$の分割である．
     ただし，
     ...
     \end{align}
     とすれば，これらは条件\ref{cad_exa_1}, \ref{cad_exa_2}を満たし，さらに$\{C_{S_i,k}\}_{k=1}^{11}$は$S_5 \times \R$の分割を与える．
     こうして$\R^2$の分割$\{C_{S_i,k}\}$が得られる．
     この$\{C_{S_i,k}\}$のように，余次元$1$の部分空間である$a$軸の分割から柱状に構成される分割を，柱状代数分解という．
     厳密な定義は\cref{section:6}で行う．
          
     \todo[inline]{この辺まではいったん考えた}

     さらに，各$C_{S_i, k}$の上で$f, g$の符号は一定である．
