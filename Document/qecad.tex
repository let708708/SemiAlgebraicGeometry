...
\begin{corollary}\label{corollary:trick-of-qecad}
     ...
     高々$X_1, \dots, X_i$を自由変数に持つ量化記号のない$\mathcal{L}_\mathrm{OR}$論理式の族$\{\theta_{i,j}(X_1, \dots, X_i)\}_{j=1}^{l_i}$が存在し，
     次を満たす．

     任意の実閉体$R$に対し，
     \begin{equation}
          \mathcal{S}_i \coloneqq\{\{(x_1, \dots, x_i) \in R^i \mid R \models \theta_{i,j}(x_1, \dots, x_i)\} \mid j=1, \dots, l_i\}
     \end{equation}
...
\begin{proof}
     ...
     \begin{equation}
          \theta_{i, \sigma}(X_1, \dots, X_i) \coloneqq \bigwedge_{f \in \overline{F}_{\leq i}}\sign(f(X_1, \dots, X_i)) = \sigma(f)
     \end{equation}
...
高々$X_1, \dots, X_i$を自由変数に持つ量化記号のない$\mathcal{L}_\mathrm{OR}$論理式の族$\{\theta_{i,j}(X_1, \dots, X_i)\}_{j=1}^{l_i}$を，
任意の実閉体$R$に対し，$\mathcal{S}_i \coloneqq \{S_R(\theta_{i,j})\}_{j=1}^{l_i}$により定まる$\{\mathcal{S}_i\}_{i=1}^n$が$R^n$の$F$適合な柱状代数分解であるように取る．
ただし，$S_R(\theta_{i,j}) \coloneqq \{(x_1, \dots, x_i) \in R^i \mid R \models \theta_{i,j}(x_1, \dots, x_i)\}$である．

ここで，$\CSIGN_i(F)(\theta_{i,j})$ ( $j=1, \dots, l_i$ ) を，
\begin{equation}
     \CSIGN_i(F)(\theta_{i,j}) \coloneqq \CSIGN_i(F)(x), \quad x \in S_{\Ralg}(\theta_{i,j})
\end{equation}
と定める．\cref{proposition:independence-of-CSIGN}より，これは$x \in S_{\Ralg}(\theta_{i,j})$の取り方に依らない．

このとき，次が成り立つ．

\begin{proposition} \todo{重要}
     任意の実閉体$R$および任意の$x \in S_R(\theta_{i,j})$に対し，
     $\CSIGN_i(F)(x) = \CSIGN_i(F)(\theta_{i,j})$である．
\end{proposition}

\todo[inline]{
     まず，任意の量化記号のない論理式$\theta(X)$と，任意の実閉体$R_1, R_2$に対して，
     $S_{R_1}(\theta) \neq \emptyset \iff S_{R_2}(\theta) \neq \emptyset$である．
     また，任意の実閉体$R_1 \subset R_2$に対して，
     $S_{R_1}(\theta) \subset S_{R_2}(\theta)$である．
     よって，$x \in S_R(\theta_{i,j})$に対し，$y \in S_{\Ralg}(\theta_{i,j})$を取れば，
     $\CSIGN_i(F)(x) = \CSIGN_i(F)(y)$となり示される．

     この証明が正しければ，このための命題を4節に載せるべきだろう．
}
...
任意の量化記号のない$\mathcal{L}_\mathrm{OR}$論理式は，ある$F \subset \Z[X_1, \dots, X_m]$を取ることで，量化記号のない$F$論理式とみなすことができることに注意する．

\begin{definition}[{\cite[Notation 11.12]{MR2248869}}]
     $F \subset \Z[X_1, \dots, X_n]$を有限部分集合とし，$\map{\sigma}{F}{\{-1, 0, 1\}}$とする．
     量化記号のない$F$論理式$\phi\left(\overline{X}\right)$に対して，$\phi^\star(\sigma) \in \{\true, \false\}$を以下のように帰納的に定義する．
     \begin{enumerate}
          \item $\phi\left(\overline{X}\right)$が$\sign_R\left(f\left(\overline{X}\right)\right) = \tau$ ( $f \in F$, $\tau \in \{-1, 0, 1\}$ ) であるとき，
          \begin{equation}
          ...
          \end{equation}
          と定義する．
          \item $\phi\left(\overline{X}\right)\coloneqq \phi_1\left(\overline{X}\right) \land \phi_2\left(\overline{X}\right)$であるとき，
          \begin{equation}
               \phi^\star(\sigma)\coloneqq \begin{cases}
          ...
          \end{equation}
          と定義する．
          \item $\phi\left(\overline{X}\right)\coloneqq \phi_1\left(\overline{X}\right) \lor \phi_2\left(\overline{X}\right)$であるとき，
          \begin{equation}
               \phi^\star(\sigma)\coloneqq \begin{cases}
          ...
          \end{equation}
          と定義する．
          \item $\phi\left(\overline{X}\right)\coloneqq \lnot \phi_1\left(\overline{X}\right)$であるとき，
          \begin{equation}
               \phi^\star(\sigma)\coloneqq \begin{cases}
...
\begin{proposition} \label{proposition:qe-cad-propoerty}
     ...
     \begin{equation}
          \Qua_1 \sigma_1 \in \CSIGN_n(F)(x) \Qua_2 \sigma_2 \in \sigma_1 \dots \Qua_m \sigma_m \in \sigma_{m-1} (\psi^\star(\sigma_m) = \true)
     \end{equation}
     が成り立つことが同値である．
\end{proposition}
...
\begin{proof}
     ...
     \begin{align}
               & R \models \phi(x)\\
          \iff & R \models \Qua_1 Y_1 \dots \Qua_m Y_m \phi(x, Y_1, \dots, Y_m)
     \end{align}
     である，
     
     $\Qua_1 = \exists$の場合，
     \begin{equation}
          R \models \exists Y_1 \Qua_2 Y_2 \dots \Qua_m Y_m \phi(x, Y_1, \dots, Y_m)
     \end{equation}
     であることの必要十分条件は，ある$y_1 \in R$について 
     \begin{equation}
          R \models \Qua_2 Y_2 \dots \Qua_m Y_m \phi(x, y_1, Y_2, \dots, Y_m)
     \end{equation}
     を満たすことである．さらに，帰納法の仮定より，これはある$y_1 \in R$について
     \begin{equation}
          \Qua_2 \sigma_2 \in \CSIGN_{n+1}(F)(x, y_1) \dots \Qua_m \sigma_m \in \sigma_{m-1} (\psi^\star(\sigma_m) = \true)
     \end{equation}
     を満たすことに必要十分である．$\CSIGN_n(F)(x) = \{\CSIGN_{n+1}(F)(x, y_1) \mid y_1 \in R\}$であるから，これは
     \begin{equation}
          \exists \sigma_1 \in \CSIGN_n(F)(x) \Qua_2 \sigma_2 \in \sigma_1 \dots \Qua_m \sigma_m \in \sigma_{m-1} (\psi^\star(\sigma_m) = \true)
     \end{equation}
     に必要十分である．
...
\begin{proposition} \label{proposition:qe-cad-method}
     ...
     $\{\theta_{i,j}(X_1, \dots, X_i)\}_{j=1}^{l_i}$  ( $i=1, \dots, n+m$ ) を，高々$X_1, \dots, X_i$を変数記号に持つような量化記号のない$\mathcal{L}_{\mathrm{OR}}$論理式の族で，
     任意の実閉体$R$に対し，$\mathcal{S}_i \coloneqq \{S_R(\theta_{i,j})\}_{j=1}^{l_i}$により定まる$\{\mathcal{S}_i\}_{i=1}^{n+m}$が$R^{n+m}$の$F$適合な柱状代数分解であるように取る．
     ただし，$S_R(\theta_{i,j}) \coloneqq \{(x_1, \dots, x_i) \in R^i \mid R \models \theta_{i,j}(x_1, \dots, x_i)\}$である．
     

     このとき，$\Theta \subset \{\theta_{n,j}\}_{j=1}^{l_n}$を，
     \begin{equation}
          \Qua_1 \sigma_1 \in \CSIGN_n(F)(\theta_{n,j}) \Qua_2 \sigma_2 \in \sigma_1 \dots \Qua_m \sigma_m \in \sigma_{m-1}(\psi^\star(\sigma_m) = \true)
     \end{equation}
     が成り立つような$\theta_{n,j}$全体とすれば，
     \begin{equation}
          \RCF \models \bigvee_{\theta_{n,j} \in \Theta} \theta_{n,j}(X_1, \dots, X_n) \leftrightarrow \phi(X_1, \dots, X_n)
     \end{equation}
     が成り立つ．
\end{proposition}
...
\begin{proof}
     任意の実閉体$R$に対して
     \begin{equation}
          \bigcup_{\theta_{n,j} \in \Theta} \{x \in R^n \mid R \models \theta_{n,j}(x)\} = \{x \in R^n \mid R \models \phi(x)\}
     \end{equation}
     が成り立つことを示せばよい．
     また，$S_R(\theta_{i,j}) = \{x \in R^n \mid R \models \theta_{n,j}(x)\}$であることに注意する．

     まず，ある$\theta_{n,j} \in \Theta$について$x \in S_R(\theta_{i,j})$
     であるとき，$\CSIGN_i(F)(x) = \CSIGN_i(F)(\theta_{i,j})$であるから，
     \cref{proposition:qe-cad-method}より$R \models \phi(x)$を満たす．

     一方，ある$R \models \phi(x)$を満たす$x \in R^n$を任意とると，
     $\{\mathcal{S}\}_{i=1}^{n+m}$は柱状代数分解であるから，
     ある$\theta_{i,j}$に対して$x \in S_R(\theta_{i,j})$である．
     また，$R \models \phi(x)$であるから，\cref{proposition:qe-cad-method}より
     \begin{equation}
          \Qua_1 \sigma_1 \in \CSIGN_n(F)(x) \Qua_2 \sigma_2 \in \sigma_1 \dots \Qua_m \sigma_m \in \sigma_{m-1} (\psi^\star(\sigma_m) = \true)
     \end{equation}
     を満たす．よって，$\CSIGN_i(F)(x) = \CSIGN_i(F)(\theta_{i,j})$であるから，
     $\theta_{i,j} \in \Theta$である．
     以上で命題が示された．
\end{proof}

\section{柱状代数分解による量化記号消去のアルゴリズム} \todo{火}
