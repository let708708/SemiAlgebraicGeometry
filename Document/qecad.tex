...
\todo[inline]{自由変数の定義のみ，正しい定義ではなかった．そのため，あまり修正していない．}
...
\begin{proposition}
     任意の量化記号のない$\mathcal{L}$論理式$\phi$に対して，有限個の$\mathcal{L}$論理式$L_{i,j}$が存在して次を満たす．
     \begin{itemize}
          \item 
          各$L_{i,j}$は，原子論理式$\psi$または原子論理式の否定$\lnot \psi$である．
          \item
          任意の$\mathcal{L}$構造$\mathcal{M}$に対して$\mathcal{M} \models \phi \leftrightarrow \bigvee_{i=1}^n \bigwedge_{j=1}^{m_i} L_{i,j}$ を満たす．
     \end{itemize}
\end{proposition}
...
\begin{proof}
     ...
     まず，$\phi$が原子論理式の場合，$\mathcal{M} \models \phi \leftrightarrow \phi$であるから命題は成り立つ．

     次に，論理記号の数が$k$個以下である場合に命題が成り立つと仮定して，論理記号の数が$k+1$個の論理式$\phi$の場合にも命題が成り立つことを示す．
     \begin{itemize}
          \item
          $\phi = \psi \lor \psi'$または$\phi = \psi \land \psi'$の場合，帰納法の仮定より，
          \begin{align}
               \mathcal{M} \models \psi \leftrightarrow \bigvee_{i} \bigwedge_j L_{i,j},\\
               \mathcal{M} \models \psi' \leftrightarrow \bigvee_{i'} \bigwedge_{j'} L'_{i',j'}
          \end{align}
          であるように原子論理式またはその否定からなる有限個の論理式$L_{i,j}$および$L'_{i,j}$を取ることができる．
          このとき，
          \begin{align}
               \mathcal{M} \models \psi \lor  \psi' \leftrightarrow \left( \bigvee_i \bigwedge_j L_{i,j} \right) \vee \left( \bigvee_{i'} \bigwedge_{j'} L'_{i',j'}\right)\\
               \mathcal{M} \models \psi \land \psi' \leftrightarrow \bigvee_{i, i'} \left(\bigwedge_j L_{i,j} \wedge \bigwedge_{j'} L'_{i',j'}\right)
          \end{align}
          であるから，$\phi$についても命題が成り立つ．

          \item
          $\phi = \lnot \psi$の場合，帰納法の仮定より，
          \begin{equation}
               \mathcal{M} \models \psi \leftrightarrow \bigvee_{i} \bigwedge_j L_{i,j},
          \end{equation}
          であるように原子論理式またはその否定からなる有限個の論理式$L_{i,j}$を取ることができる．
          このとき，
          \begin{equation}
               \mathcal{M} \models \lnot \psi \leftrightarrow \bigvee_{j_1, \dots, j_n} \bigwedge_i \overline{L}_{i,j_i}
          \end{equation}
          が成り立つ．
          ただし，$\overline{L}_{i,j}$は$L_{i,j}$が原子論理式$\theta$のときは$\overline{L}_{i,j}:=\lnot \theta$，
          $L_{i,j}$が原子論理式の否定$\lnot \theta$のときは$\overline{L}_{i,j}:=\theta$である．
     \end{itemize}
     したがって，帰納法により命題が示された．
\end{proof}
...
\begin{proposition}
     任意の$\mathcal{L}$論理式$\phi(w_1, \dots, w_n)$に対し，ある量化記号のない論理式$\psi(v_1, \dots, v_m, w_1, \dots, w_n)$及び量化記号$\Qua_1, \dots, \Qua_m \in \{\forall, \exists\}$が存在して，
     任意の$\mathcal{L}$構造$\mathcal{M}$に対し
     \begin{equation}
          \mathcal{M} \models \phi(w_1, \dots, w_n) \leftrightarrow \Qua_1 v_1 \dots \Qua_m v_m \psi(v_1, \dots, v_m, w_1, \dots, w_n)
     \end{equation}
     を満たす．$\Qua_1 v_1 \dots \Qua_m v_m \psi(v_1, \dots, v_m, w_1, \dots, w_n)$のような形の論理式を冠頭標準形の論理式という．
\end{proposition}
...
\begin{theorem}[{\cite[Theorem 11]{MR0044472}}]\label{theorem:Tarski}
     言語$\mathcal{L}_\mathrm{OR}$上の実閉体の理論$\RCF$は量化記号消去をもつ．
\end{theorem}

以降この節は\cref{theorem:Tarski}の証明を述べる．

この定理の証明および後々の議論のため，言語$\mathcal{L}_\mathrm{OR} = \{<, +, -, \cdot, 1, 0\}$上の原子論理式について注意をする．

さらに，$x_1, \dots, x_n$を変数記号とする．多項式$f \in \Z[x_1, \dots, x_n]$が与えられるとき，$f(x_1, \dots, x_n)$は$\mathcal{L}$項として自然に解釈できる．
...
\begin{proof}
     ...
     と$\map{\sigma_i}{\{1, \dots, l_i\}}{P}$を用いて，和積標準形\todo[inline]{ 「を用いて，表す」は違和感ない．和積標準形は定義していない．}
     \begin{equation}
...
\begin{proof}
     ...
     ここで，$i_0 = 0, i_{M+1} = N+1$としておき，また，$x_0= -\infty, x_{N+1} = \infty$とする．
