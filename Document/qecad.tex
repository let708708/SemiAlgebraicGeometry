...
\section{基礎的な定義}
% コンピュータによって「具体的な問題の」量化子消去を行うというモチベーションで議論を進める．
% すると実数体を扱えないことから，実数体の公理を抜き出す動機が生まれそう？

\subsection{実閉体}
% このセクションでは，実数体の公理を抜き出し，順序体を定義する．一階述語論理でも「順序体の言語」を用いる．
% また，実数体の公理を抜き出すことで，実体および実閉体を定義する．

\subsection{一階述語論理}
%このセクションでは，一階述語論理の用語を定義する．
%必要なもの：項，論理式，モデル(ここまで書いた)，理論，解釈…………
...
\begin{example}[順序体の言語]
     次の記号からなる言語$L_\mathrm{OR}$を，順序体の言語という．
     \begin{itemize}
          \item 2変数述語記号 $<$,
          \item 2変数関数記号 $+$, $\cdot$,
          \item 定数記号 $1$, $0$.
...
% ここで自由変数の定義をする．
% 順序体の例を用いてモデルを導入するとよりよさそう．

\begin{definition}[モデルの定義]
     言語$L$のモデルとは，空でない集合$M$と，次のようなデータの組$\mathfrak{M}=(M,\dots)$である．
     \begin{itemize}
