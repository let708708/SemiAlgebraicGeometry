...
\section{導入}\label{section:1}
     ...
     このような$a$軸の分割$\{S_i\}_{i=1}^9$をとると，\todo{連続関数の大小は定義していない．}
     各$S_i$に対し有限個の連続関数$\map{\xi_{S_i,1} < \cdots ＜ \xi_{S_i,l_i}}{S_i}{\R}$が存在して，
     任意の$a \in S_i$に対し$\{\xi_{S_i,k}(a)\}_{j=1}^{l_i}$は$x$変数の多項式$f(a,x)$, $g(a,x)$の根全体であるようにとれる．

     \todo[inline]{このとしては定義である．}
