...
\todo[inline]{量化記号消去と柱状代数分解がどのように関連するかについて言及する．}

まず，記号について定義する．
以下の定義は，\cite[Notation 11.7.]{MR2248869}を参考にした．
\begin{definition}
     $F \subset R[X_1, \dots, X_n]$とする．
     ...
     \begin{itemize}
          \item $i=k$のとき，$x \in R^i$に対し，$\CSIGN_k(F)(x):= \sign(F)(x) $とする．
          \item $0 \leq i<k$のとき，$y \in R^i$に対し，
          \begin{equation}
...
\todo[inline]{以下のあたりの書き方がマジでよくわからない．😢}
     ...
     \begin{algorithmic}[1]
          ...
          \FOR{$i = 2, \dots, k$}
               \FOR{$x \in A_{i-1}$}
                    ...
                    \STATE $A_i \leftarrow A_i + \{(x, y)\mid y \in \widetilde{A}_i\}$
               \ENDFOR
          \ENDFOR
...
\begin{algorithm}
     ...
     \begin{algorithmic}[1]
          ...
          \STATE $Y_1, \dots, Y_l, X_1, \dots, X_k$と$F$に対して，Improved Cylindrical Decompositionを適応する．
          \FOR{$\sigma \in \overline{C}_{\leq l}(F)$}
               \IF{$\Reali(\sigma) \neq \emptyset$}
                    \STATE $\CSIGN_l(F)(\sigma)$を，
               \ENDIF
          \ENDFOR
     \end{algorithmic}
\end{algorithm}
