...
\begin{proof}
     ...
     \cref{proposition:dnf}から，%本来は\cref{proposiiton:dnf}を少しだけ改良したものである．
...
\begin{remark}
     ...
     \begin{itemize}
          \item
               ...
               $S$は$R^n$の半代数的集合である．
          \item 
               $S \subset R^n$が$R^n$の半代数的集合ならば，$R^n \setminus S$も$R^n$の半代数的集合である．
          \item 
               $S_1, S_2 \subset R^n$が$R^n$の半代数的集合ならば，$S_1 \cup S_2$, $S_1 \cap S_2$も$R^n$の半代数的集合である．
     \end{itemize}
\end{remark}
...
\begin{proof}
     ...
     各$\pi_n(S_i)$が$R^n$の半代数的集合であることを示せばよい．
     従って，あらかじめ$S$が有限個の$f_j \in R[\overline{X}, Y]$および$\sigma_j \in \{-1, 0, 1\}$, $j=1, \dots, l$, を用いて
     \begin{equation}
          S = \bigcap_{j=1}^{l} \{(\overline{x},y) \in R^n \mid \sign_R(f_j(\overline{x},y)) = \sigma_j\}
     \end{equation}
     と書けるとして，$\pi_n(S)$が$R^n$の半代数的集合であることを示せばよい．
...
% また，$\mathfrak{D}$が$\R^{n-1}$の分割であるとき，$\mathfrak{L}(\mathfrak{D}) \coloneqq \bigcup_{D \in \mathfrak{D}}\mathfrak{L}(D)$を$\mathfrak{D}$の持ち上げという．
% \end{definition}
