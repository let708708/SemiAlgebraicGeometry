...
% このとき，$ S $上の$ F $の解を$ f_1(x)< \dots <f_k(x) $とし，$ f_0(x) \coloneqq -\infty $ , $ f_{k+1}(x) \coloneqq \infty $とするとき，
% \begin{align}
%   C_{2i} &\coloneqq \{(x,y) \mid  x \in S, f_i(x) = y \} \quad i = 1,\dots, k,\\
%   C_{2i+1} &\coloneqq \{(x,y) \mid x \in S, f_{i}(x)<y<f_{i+1}(x) \} \quad i = 0,1, \dots, k 
% \end{align}

% とすれば，$\{C_j\}_{j=1}^{2k+1}$は$ S \times \mathbb{R} $の$F$符号不変な分解を与える．
% この$ S \times \mathbb{R} $の分解$ \{C_j\}_{j=1}^{2k+1} $を，$ S $の持ち上げといい，$ \mathfrak{L}(S) $と書く．

% また，$\mathfrak{D}$が$\R^{n-1}$の分解であるとき，$\mathfrak{L}(\mathfrak{D}) \coloneqq \bigcup_{D \in \mathfrak{D}}\mathfrak{L}(D)$を$\mathfrak{D}$の持ち上げという．
% \end{definition}
