...
\section{導入}\label{section:1}
     ...
     \end{equation}
     に必要十分である．このように論理式$\phi(a,b)$を同値な論理式$\theta(a,b)$に言い換えれば，
     ...
     さて，上記で得られた$\R^2$の柱状代数分解$\{C_{S_i,k}\}$を用いて，$\phi(a)$の量化記号消去を行おう．
     この$\{C_{S_i,k}\}$は，各$C_{S_i,k}$上で$f, g$の符号は一定である．
     すると各$S_i$について，$a \in S_i$が$\phi(a)$を満たすことは，
     ある$k=0, \dots, 2l_i+1$に対して$C_{S_i,k}$上$f<0, g<0$となることと同値である．
     すなわち，次の等式が成り立つ．
     \begin{equation}
