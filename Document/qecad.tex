...
\section{導入}\label{section:1}
     ...
     \begin{figure}[H]
          ...
          \caption{\cref{question:ut2021}の解答}
               \begin{tikzpicture}[scale=1]
                    ...
                    \fill [pattern = north east lines] (-2,0)--(2,0)--(0,-2);
               \end{tikzpicture}
          \label{fig:pic0}
     \end{figure}
     ...
     \begin{figure}[H]
          ...
          \begin{tikzpicture}[scale=1.5]
               ...
               \begin{scope}
                    ...
                    \fill[pattern = north east lines] (-2,-2)--(2,2)--(2,-2);
               \end{scope}
          \end{tikzpicture}          
     ...
     ただし，領域$f(a,x)<0$, $g(a,x)<0$は図の灰色部分である．
     ...
     次に，この4点の$a$座標を境界にして，$a$軸を次のような$9$つの開区間及び$1$点からなる集合$\{S_i\}_{i=1}^9$に分割する．
     \begin{itemize}
          \item $S_1 \colon a < -1$,
          \item $S_2 \colon a = -1$,
     ...
     本論文では，このようなアルゴリズムを，実数体を公理化した実閉体上で一般の論理式に対して行う．
...
\section{実閉体上の量化記号消去の背景}
実閉体上の量化記号消去のアルゴリズムは，初めにTarski\cite{MR0044472}によって述べられた．
しかし，Tarskiによる量化記号消去の実行時間は，指数関数の有限回の冪で抑えることはできなかった．
\footnote{"The running time, however, of the method from Tarski (1951) cannot be bounded by any finite tower of exponential functions.\cite{MR0949113}"}
後に，Collins\cite{MR0403962}は柱状代数分解(CAD)を用いた新しい量化記号消去のアルゴリズムを提案した．
...
その後の大きな発展は，CollinsとHong\cite{MR1128247}によるPartial CADを用いた量化記号消去の登場である．
Hongらは，CADの構成を行うアルゴリズムを，量化記号消去において必要な箇所のみ行うように改良したことで
より高速な量化記号消去が行えるようになった．さらに，HongらはこのアルゴリズムをSACLIB上に$\mathtt{qepcad}$として実装した．
