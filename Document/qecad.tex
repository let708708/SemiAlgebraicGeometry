...
\begin{titlepage}
     ...
     {\LARGE
       ...
       \centerline{修士論文題目}%
       \vspace{10pt}%
       \centerline{\underline{実閉体上の柱状代数分解による量化記号消去}}}%
     \vfill
     {\Large
       \centerline{%
         \begin{tabular}{c@{\hspace{1zw}}l}%
           \kintou{5zw}{学生証番号} & \underline{45-216021} \\
           \kintou{5zw}{フリガナ} & トミナガ ナオヤ \\
           \kintou{5zw}{氏名} & \underline{富永 直弥} \\
         \end{tabular}}}%
     \vspace{60pt}%
\end{titlepage}
...
\section{導入}\label{section:1}
     ...
     Tarski \cite{MR0044472}は，実閉体の理論が量化記号消去を持つことを構成的に示した．
     ...
     \begin{figure}[H]
          \centering
               \begin{tikzpicture}[scale=1]
                    ...
                    \draw[very thick, domain=-3:2] plot(\x,-\x-2);
                    \draw[very thick, domain=-2:3] plot(\x,\x-2);
     ...
     実数の四則演算と大小関係に関する任意の論理式に対して量化記号消去ができるということは
     Tarski \cite{MR0044472}によって示されていたが，
     Collins \cite{MR0403962}がより実用的な柱状代数分解を用いたアルゴリズムを提案した．
     ...
     \begin{figure}[H]
          ...
          \begin{tikzpicture}[scale=1.5]
               ...
               \draw[very thick] (0,0) circle(1 and 1);
               \draw (0,0) node[above left]{O};
     ...
     \begin{itemize}
          \item $S_1 \coloneqq \{a \in \R \mid a < -1\}$,
          \item $S_2 \coloneqq \{a \in \R \mid a = -1$\},
          \item $S_3 \coloneqq \{a \in \R \mid -1 < a < -1/\sqrt{2}\}$,
          \item $S_4 \coloneqq \{a \in \R \mid a = -1/\sqrt{2}\}$,
          \item $S_5 \coloneqq \{a \in \R \mid -1/\sqrt{2} < a < 1/\sqrt{2}\}$,
          \item $S_6 \coloneqq \{a \in \R \mid a = 1/\sqrt{2}\}$,
          \item $S_7 \coloneqq \{a \in \R \mid 1/\sqrt{2} < a < 1\}$,
          \item $S_8 \coloneqq \{a \in \R \mid a = 1\}$,
          \item $S_9 \coloneqq \{a \in \R \mid 1 < a\}$.
     \end{itemize}
     これらの集合によって$a$軸を分割し，\cref{fig:pic2}に合わせて描画すると次のようになる．
     ...
     \begin{figure}[H]
          ...
          \begin{tikzpicture}[scale=1.5]
               ...
               \draw[very thick] (-{sqrt(1/2)},-2)--(-{sqrt(1/2)},2);
               \draw[very thick] ({sqrt(1/2)},-2)--({sqrt(1/2)},2);
               \draw[very thick] (1,-2)--(1,2);
               \fill (-1,0) circle(0.05 and 0.05);
               \fill (1,0) circle(0.05 and 0.05);
               ...
               \draw[->, >=stealth, semithick] (-2,-3)--(2,-3) node[right]{$a$};
               \draw[dashed] (-1,-3)--(-1,-2);
               \draw[dashed] (-{sqrt(1/2)},-3)--(-{sqrt(1/2)},-2);
               \draw[dashed] ({sqrt(1/2)},-3)--({sqrt(1/2)},-2);
               \draw[dashed] (1,-3)--(1,-2);
               \fill (-1,-3) circle(0.05 and 0.05);               
               \fill (-{sqrt(1/2)},-3) circle(0.05 and 0.05);     
               \fill ({sqrt(1/2)},-3) circle(0.05 and 0.05);     
               \fill (1,-3) circle(0.05 and 0.05);
          \end{tikzpicture}

          \caption{$a$軸の分割$\{S_i\}_{i=1}^9$を描画}
          \label{fig:pic3}
     \end{figure}
     ...
     を満たすようにとることができ，このとき
     \begin{align}
          \label{align:intro-1} C_{S_i, 2j} &\coloneqq \{(a,\xi_{S_i, j}(a)) \in \R^2 \mid a \in S_i\}, \quad j=1,...,l_i,\\
     ...
     ただし，
     $\xi_{S_i,0} \coloneqq -\infty$, $\xi_{S_i,l_i+1} \coloneqq \infty$と置く．
     ...
     であるため，
     \begin{equation}
          \theta(a) \coloneqq (-1/\sqrt{2} < a < 1/\sqrt{2}) \lor (a = 1/\sqrt{2}) \lor (1/\sqrt{2} < a < 1)
     \end{equation}
     ...
     \cref{section:4}で理論が量化記号消去を持つことを定義し，実閉体の理論が量化記号消去を持つことを示す．
     
     そして，\cref{section:5}では，実閉体上の半代数的集合を定義しその性質についてまとめる．この概念は，\cref{section:6}で定義する柱状代数分解に必要である．
     \cref{section:6}では，実閉体を係数とする多項式の有限部分集合$F$に対し，$F$適合な柱状代数分解を定義してその存在を示す．
...
また，複素数体$\C$は$\left(\sqrt{-1}\right)^2 = -1$を満たす元$\sqrt{-1}$を持つ．
従って，$\C$は実体ではない．
...
\begin{definition}
     ...
     \begin{enumerate}
          ...
          と定義する．
          \item 論理式$\phi$に対して，$\Var(\phi)$が定義されているとき，変数記号$v_i$に対して
          $\Var(\forall v_i\phi)\coloneqq\Var(\phi) \setminus \{v_i\}$,
          $\Var(\exists v_i\phi)\coloneqq\Var(\phi) \setminus \{v_i\}$
          と定義する．
     \end{enumerate}
     自由変数でない変数を，束縛変数(bound variable)という．
     $\mathcal{L}$論理式$\phi$に登場する自由変数が高々$v_{i_1}, \dots, v_{i_m}$である事を明示したいとき，$\phi(v_{i_1}, \dots, v_{i_m})$と書く．ただし，登場しない自由変数があっても良い．
...
\begin{definition}[{\cite[Section 1.2]{MR1924282}}]
     ...
     さらに，$\mathcal{L}$構造$\mathcal{M}$が任意の$\phi \in T$に対し
     $\mathcal{M} \models \phi$を満たすとき，$\mathcal{L}$構造$\mathcal{M}$を$\mathcal{L}$理論$T$のモデル(model)といい
     $\mathcal{M} \models T$と書く．
\end{definition}
...
\begin{definition}[実閉体の理論]
     ...
     以下の$\mathcal{L}_\mathrm{OR}$文全体からなる集合として定義する．
          \begin{enumerate}
               ...
               \item $\forall x_{2n} \dots \forall x_1 \forall x_0 \exists y (y^{2n+1} + x_{2n} \cdot y^{2n} + \dotsb + x_1 \cdot y + x_0 = 0)$ ( $n =  1, 2 , \dots$ ).
          \end{enumerate}

     ここで，項$t$に対し，$t^p$は$t$の$p$個の積$(t\cdot(t\cdot(\dotsb \cdot t)))$の省略とする．
\end{definition}

\begin{remark}以下の点に注意する．
     \begin{itemize}
          \item 
          定数記号$1$の$p$個の和$(1+(1 + (\dotsb + 1)))$の省略として$p$を用いる．
          \item 
...
\begin{definition}[{\cite[Definition 1.2.12]{MR1924282}}]
     ...
     $\phi$が$T$の論理的帰結であるとき，$T \models \phi$と書く．
...
\begin{proof}
     ...
     任意の量化記号のない論理式$\phi\left(X, \overline{Y}\right)$に対して，
     多項式$f_{i,j} \in \Z\left[X, \overline{Y}\right]$ ($i=1, \dots, k, j=1, \dots, l_i$)
...
\begin{proof}
     ...
     \begin{itemize}
          ...
          \end{equation}
          \item $M\neq0$の場合，$(-\infty, x_1)$に解を持つことの必要十分条件は，
          \begin{equation}
               \sign_R(f_s'((-\infty, x_1))) \sign_R(g_{\theta(1)}(x_1)) = 1.
          \end{equation}
          \item $M\neq0$の場合，$(x_M, \infty)$に解を持つことの必要十分条件は，
          \begin{equation}
...
\begin{proof}[\cref{proposition:weak_Tarski}の証明]
     ...
     非負整数列全体$\{(m_1, \dots, m_s) \mid s \geq 1, m_1, \dots, m_s \geq 0 \}$上の前順序関係
     \begin{equation}
     ...
     \begin{itemize}
          ...
          \item 任意の$q>p$に対して，$q$に等しい$m_i$ ($i=1, \dots, s$)の数と，$q$に等しい$m'_i$ ($i=1, \dots, s'$)の数は等しい．
     \end{itemize}
...
\begin{proof}
     \cref{proposition:dnf}より，ある有限個の$F_{i,j} \in \Z\left[X, T\right]$および$\sigma_{i,j} \in \{-1, 0 ,1\}$, 
     $i=1, \dots, s, j = 1, \dots, l_i$, が存在して，
     \begin{equation}
          R \models
...
\begin{proof}
     \cref{theorem:Tarski}より，ある量化記号のない$\mathcal{L}_\mathrm{OR}$論理式$\theta(X,T)$が存在し，
     \begin{equation}
          R \models \phi(X,T) \leftrightarrow \theta(X, T)
     \end{equation}
     が成り立つ．従って，
     \begin{equation}
          S = \{ x \in R^n \mid R \models \theta(x, t) \}
...
\begin{proof}[\cref{theorem:Tarski-Seidenberg}の証明]
     ...
     ある量化記号のない$\mathcal{L}_\mathrm{OR}$論理式$\theta(X, T)$および$t \in R^k$を用いて，
     \begin{equation}
          S = \{(x_1, \dots, x_n, x_{n+1}) \in R^{n+1} \mid R \models \theta(x_1, \dots, x_n, x_{n+1}, t)\}
     \end{equation}
     と表すことができる．このとき，
     \begin{align}
          \pi_n(S) = \{(x_1, \dots, x_n) \in R^n \mid  R \models \exists X_{n+1} \theta(x_1, \dots, x_n, X_{n+1}, t)\}
     \end{align}
...
\begin{proof}
     ...
     \end{equation}
     が成り立つように取れる．
     これより，$\RCF \models \theta$であるか，$\RCF \models \lnot \theta$のいずれか片方のみを満たす．
...
\begin{definition} 
     ...
     $S$が$F$描画可能($F$-delineable)であるとは，有限個の連続な半代数的関数$\map{\xi_1<\dots<\xi_k}{S}{R}$が存在し，次を満たすことと定義する．
     \begin{itemize}
          ...
          \item 各$i=1, \dots, k$および$f \in F'$に対し，$X_n$に関する多項式$f(x,X_n)$の根$\xi_i(x)$の重複度は，$x\in S$に依らず一定である．
     \end{itemize}
\end{definition}
...
\begin{proof}
     $ a \in S$を任意にとる．
     また，$ F' \coloneqq \{f \in F \mid \text{$f$は恒等的に$0$でない}\}$とする．
...
\begin{definition}[{\cite[Section 2]{MR0403962}}]
     ...
     また，$M_{j,j}(A,B)$を，$M_j(A,B)$の初めの$m+n-2j$列により得られる$(m+n-2j)$行$(m+n-2j)$列行列とし，
...
\cref{theorem:psc}は，$\deg(\gcd(A,B))$が主部分終結式係数$\psc_j(A, B)$により決定することを意味している．
\cref{theorem:psc}の証明は\cite[Theorem 2]{MR0403962}を見よ．

実閉体$R$と$f, g \in R[X_1, \dots, X_n] \setminus \{0\}$に対し，
$f,g$を$R[X_1, \dots, X_{n-1}]$係数の$X_n$変数多項式とみなすことで
$f$と$g$の主部分終結式係数$\psc_j(f,g)$が定まる．
これを$\psc_j(f,g;X_n)$と明示する．
...
とし，
\begin{equation}
     \RED(f;X_n) \coloneqq \{\red^k(f;X_n) \mid 0 \leq k \leq \deg(f;X_n), \red^k(f;X_n) \neq 0\}
\end{equation}
と定義する．
さらに，$f, g \in R[X_1, \dots, X_n] \setminus \{0\}$に対し，$\deg(f;X_n) = m$, $\deg(g;X_n) = n$とするとき，
\begin{equation}
     \PSC(f,g;X_n) \coloneqq \{ \psc_j(f,g;X_n) \mid 0 \leq j \leq \min\{n,m\}, \psc_j(f,g;X_n) \neq 0\}
\end{equation}
と定義する．$f = 0$または$g=0$の場合，$\PSC(f,g;X_n) \coloneqq \emptyset$と定義する．

次の定義は，\cite[Section 3]{MR0764184}, \cite[Section 2]{MR0403962}を参照した．
\begin{definition}
$F \subset R[X_1, \dots, X_n]$を有限部分集合とする．ただし，$n \geq 2$とする．
$\PROJ_{X_n}(F) \subset R[X_1, \dots, X_{n-1}]$を，以下の集合の合併として定義する．
\begin{align} 
     &  \bigcup_{\substack{f \in F \\ A \in \RED(f;X_n)}} \{\LC(A;X_n)\},\\
     &  \bigcup_{\substack{f \in F \\ A \in \RED(f;X_n)}} \PSC(A, \partial A / \partial X_n;X_n),\\
     &  \bigcup_{\substack{f \neq g \in F \\ A \in \RED(f;X_n) \\ B \in \RED(g;X_n)}} \PSC(A,B;X_n).
\end{align}
\end{definition}

$\PROJ_{X_n}(F)$は$0$を含まないことに注意する．
この記号のもとで，次の定理が成り立つ．
...
\begin{proof}
     ...
     このとき，$S$は$\PSC(A, \partial A / \partial X_n;X_n)$符号不変であるから，
     ...
     最後に，各$f \neq g \in F$に対し$\deg(\gcd(f(x, X_n),g(x,X_n)))$が$x \in S$に依らず一定であることを示す．
     ...
     このとき，$S$は$\PSC(A, B;X_n)$符号不変であるから，
...
\begin{proof}[\cref{theorem:cad}の証明]
     ...
     \begin{itemize}
          ...
          \item 各$i=1, \dots, l_S$および$f \in F'$に対し，多項式$f(x,X_n)$における根$\xi_{S,i}(x)$の重複度は，$x\in S$に依らず一定．
     \end{itemize}
     であるように取れる．
...
$F$適合な柱状代数分解$\{\mathcal{S}_i\}_{i=1}^n$が存在することはすでに示したが，
...
ここでは，前節で与えた$\PROJ_{X_n}(F)$を改良し，定義式まで明らかであるような柱状代数分解を与える．
...
また，多項式の有限部分集合$F \subset R[X] \setminus \{0\}$が，
...
\begin{lemma}[{\cite[Lemma 5.33]{MR2248869}}]\label{lemma:Thom}
     ...
     $\Reali_R(\sigma)$は空集合，$1$点からなる集合，開区間のいずれかである．
\end{lemma}
...
\begin{proof}
     ...
     $\Reali_R(\sigma_0)$は，空集合，$1$点からなる集合，開区間のいずれかである．

     $\Reali_R(\sigma) = \Reali_R({\sigma_0}) \cap \{x \in R \mid \sign_R(f_{s+1}(x)) = \sigma(f_{s+1})\}$であるから，
     $\Reali_R({\sigma_0})$が空集合，あるいは$1$点からなる集合の場合には，$\Reali_R(\sigma)$は空集合か$1$点からなる集合になるので主張は成り立つ．

     従って，$\Reali_R({\sigma_0})$が開区間である場合を考える．
     $f_{s+1}$の微分$f_{s+1}'$は，$f_1, \dots, f_s$のいずれかであるから，$\sign_R(f'_{s+1}(x))$は$x \in \Reali_R({\sigma_0})$に依らず一定である．
     ...
     このとき\cref{corollary:monotone}より，$f$は$\Reali_R({\sigma_0})$上狭義単調増加(あるいは狭義単調減少)であるから，$\Reali_R(\sigma)$は空集合，$1$点からなる集合，開区間のいずれかである．
...
\begin{theorem}[{\cite[Theorem 5.34]{MR2248869}}]\label{theorem:improved-cad}
     ...
     \begin{itemize}
          ...
          \item 各$i=1,\dots, n$に対して，$F_i \subset \Z[X_1, \dots, X_i]$であり，$F_i$は$X_i$の微分について閉じている．\\
...
\begin{proof}
     ...
     \begin{itemize}
          ...
          \item 各$i=1, \dots, l_S$および$f \in F_n'$に対し，多項式$f(x,X_n)$における根$\xi_i(x)$の重複度は，$x\in S$に依らず一定．
     \end{itemize}
     また，$\xi_{S,0} \coloneqq -\infty$, $\xi_{S,l_S} \coloneqq \infty$とする．
     ...
     $S \in \mathcal{S}_{n-1}$を任意に取り，
     $S = \Reali_R(\widetilde{\sigma})$である$\map{\widetilde{\sigma}}{F_{\leq n-1}}{\{-1,0,1\}}$を取る．
     このとき，
     \begin{equation}
     ...
     \end{equation}
     となることを示す．$S \times R$は，両辺の集合族の非交和であることに注意する．
     ...
     また，このような$\sigma$に対して，\cref{lemma:Thom}より，
     任意の$i,j \in \{1, \dots, 2l_S + 1\}$について
     \begin{equation}
          C_{S,i}, C_{S,j} \subset \Reali_R(\sigma) \implies i=j 
     \end{equation}
     である．
...
\begin{equation}
     \DER(F;X_n) \coloneqq \{\partial^k f / \partial X_n^k \in R[X_1, \dots, X_n] \mid f \in F, k \geq 0, \partial^k f / \partial X_n^k \neq 0\}
\end{equation}
と定める．
...
\begin{corollary} \label{corollary:improved-cad}
     ...
     \begin{align}
          \overline{F}_n & \coloneqq \DER(F;X_n),\\
          \overline{F}_i & \coloneqq \DER\left(\PROJ_{X_{i+1}}\left(\overline{F}_{i+1}\right); X_i\right), \quad i=1, \dots, n-1.
     \end{align}
...
\begin{proof}
     ...
     \end{equation}
     とすれば，\cref{corollary:improved-cad}より主張が従う．
\end{proof}
...
まず，$\mathcal{L}_\mathrm{OR}$論理式$\phi(X)$を任意にとる．
ただし，変数記号の列$X_1, \dots, X_n$を$X$と書く．
...
ここでは，$\phi (X)$ が冠頭標準形，すなわち，
ある量化記号のない$\mathcal{L}_\mathrm{OR}$論理式$\psi(X, Y_1, \dots, Y_m)$と$\Qua_i \in \{\forall, \exists\}$ ( $i=1, \dots, m$ )を用いて
\begin{equation}
     \theta(X) = \Qua_1 Y_1 \dots \Qua_m Y_m \psi(X, Y_1, \dots, Y_m)
\end{equation}
...
\begin{definition}[{\cite[Notation 11.7]{MR2248869}}]
     ...
     まず，各$y \in R^n$に対し，$F$の符号$\map{\sign_R(F)(y)}{F}{\{-1,0,1\}}$を
     \begin{equation}
          \sign_R(F)(y)(f)\coloneqq \sign_R(f(y)),\quad f \in F
     \end{equation}
     により定義する．
...
\begin{proof}
     ...
     次に，$i=k+1$において命題が成り立つとして，$i=k$の場合に示す．
     $S \in \mathcal{S}_k$を任意に取る．
     \begin{equation}
          \mathcal{C} \coloneqq \{T \in \mathcal{S}_{k+1} \mid \pi_{k}(T) = S\}
     \end{equation}
     とすると，
     帰納法の仮定より，各$T \in \mathcal{C}$において$\CSIGN_{R,k+1}(F)(x)$は$x \in T$に依らず一定である．
     このとき，各$y \in S$に対し，
     \begin{align}
          \CSIGN_{R,k}(F)(y)
...
ただし，$S_R(\theta) \coloneqq \{(x_1, \dots, x_i) \in R^i \mid R \models \theta(x_1, \dots, x_i)\}$である．
このような$\Theta_i$が存在することは\cref{corollary:trick-of-qecad}より示される．

このとき，次が成り立つ．

\begin{proposition}\label{proposition:CSIGN}
     $\theta \in \Theta_i$とする．
     $\CSIGN_{R,i}(F)(x)$は実閉体$R$および$x \in S_R(\theta)$に依らず一定である．
     すなわち，任意の実閉体$R_1$, $R_2$及び$x \in S_{R_1}(\theta)$, $y \in S_{R_2}(\theta)$に対し
     \begin{equation}
          \CSIGN_{R_1,i}(F)(x) = \CSIGN_{R_2, i}(F)(y)
     \end{equation}
     が成り立つ．
\end{proposition}

\begin{proof}
     \cref{proposition:independence-of-CSIGN}より，
     任意の実閉体$R$に対し$\CSIGN_{R,i}(F)(x)$は$x \in S_R(\theta)$に依存せず一定である．
     また，任意の実閉体$R$は$\Ralg \subset R$であるから，$S_{\Ralg}(\theta) \subset S_R(\theta)$である．
     さらに，\cref{proposition:Artin-Lang}より，任意の実閉体$R$に対し
     $S_{\Ralg}(\theta) \neq \emptyset \iff S_R(\theta) \neq \emptyset$である．
     
     従って，$y \in S_{\Ralg}(\theta)$を$1$つ取れば，
     任意の実閉体$R$および任意の$x \in S_R(\theta)$に対して
     \begin{equation}
          \CSIGN_{R,i}(F)(x) = \CSIGN_{\Ralg, i}(F)(y)
     \end{equation}
     が成り立つ．よって，命題が示された．
\end{proof}

ここで，$\theta \in \Theta_i$に対し$\CSIGN_i(F)(\theta)$ を，実閉体$R$と$x \in S_R(\theta)$を任意に取り，
\begin{equation}
     \CSIGN_i(F)(\theta) \coloneqq \CSIGN_{R, i}(F)(x)
\end{equation}
と定める．\cref{proposition:CSIGN}より，これは実閉体$R$および$x \in S_R(\theta)$の取り方に依らない．

$F \subset \Z[X_1, \dots, X_n]$を有限部分集合とする．変数記号の列$X_1, \dots, X_n$を$X$と書く．
$\sign(f(X)) = \tau$ ( $f \in F$, $\tau \in \{-1, 0, 1\}$ ) から
論理記号$\land, \lor, \lnot$によって生成される$\mathcal{L}_\mathrm{OR}$論理式を，量化記号のない$F$論理式($F$-quantifier free formula)と呼ぶ．
任意の量化記号のない$\mathcal{L}_\mathrm{OR}$論理式は，ある$F \subset \Z[X_1, \dots, X_n]$を取ることで，量化記号のない$F$論理式とみなせることに注意する．

\begin{definition}[{\cite[Notation 11.12]{MR2248869}}]
     $F \subset \Z[X_1, \dots, X_n]$を有限部分集合とし，$\map{\sigma}{F}{\{-1, 0, 1\}}$とする．
     量化記号のない$F$論理式$\psi(X)$に対して，$\psi^\star(\sigma) \in \{\true, \false\}$を以下のように帰納的に定義する．
     \begin{enumerate}
          \item $\psi(X)$が$\sign_R(f(X)) = \tau$ ( $f \in F$, $\tau \in \{-1, 0, 1\}$ ) であるとき，
          \begin{equation}
          ...
          \end{equation}
          と定義する．
          \item $\psi(X)\coloneqq \psi_1(X) \land \psi_2(X)$であるとき，
          \begin{equation}
               \psi^\star(\sigma)\coloneqq \begin{cases}
          ...
          \end{equation}
          と定義する．
          \item $\psi(X)\coloneqq \psi_1(X) \lor \psi_2(X)$であるとき，
          \begin{equation}
               \psi^\star(\sigma)\coloneqq \begin{cases}
          ...
          \end{equation}
          と定義する．
          \item $\psi(X)\coloneqq \lnot \psi_1(X)$であるとき，
          \begin{equation}
               \psi^\star(\sigma)\coloneqq \begin{cases}
...
実閉体$R$が$\psi(x)$を満たすことと$\psi^\star(\sign_R(F)(x)) = \true$であることは同値である．
...
関数$\mathtt{PROJ}$は，有限部分集合$F \subset \Z[X_1, \dots, X_n]$に対し，$\PROJ_{X_n}(F) \subset \Z[X_1, \dots, X_{n-1}]$を返す関数である．
また，関数$\mathtt{DER}$は，有限部分集合$F \subset \Z[X_1, \dots, X_n]$に対し，$\DER(F;X_n) \subset \Z[X_1, \dots, X_n]$を返す関数である．

次に定義する関数$\mathtt{ImprovedCAD}$は，有限部分集合$F \subset \Z[X_1, \dots, X_n]$に対し，
...
\end{itemize}
ただし，$\Ralg^0 = \{\mathord{*}\}$とみなすことで$n=0$も許す．
...
$n$次元の標本$\beta$を返す関数$\mathtt{DECOMP}$を次のように定める．
...
$\CSIGN_n(F)(b)$を返す関数$\texttt{CSIGN}$を，次のように定める．
