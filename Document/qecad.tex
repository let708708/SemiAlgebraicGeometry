...
まず，\cite{Artin}でArtinとSchreierにより導入された，実閉体の概念についてまとめる．
...
% $\Q(\sqrt{2})$を$\Q\left(\sqrt{2}\right)$に修正．以下同様．
...
% $\C$が順序体の構造を持たないことは命題として述べてもいいのでは．(まだ直してない)
...
% 引用の書き方が分からないのですが，これでいいでしょうか？
実閉体は\cite{Artin}により初めて定義された．
なお，\cite[Definition1.2.1]{Bochnak}の定義を参照している．
...
\begin{theorem}
     ...
     \begin{enumerate}
          ...
          \item 次を満たす$F$の順序体の構造$<$がただ一つ存在する．
               ...
               \end{itemize}
          \item $F\left[\sqrt{-1}\right] := F[X]/(X^2 + 1)$は代数的閉体である．\label{proposition:rcf_2}
     \end{enumerate}
\end{theorem}
...
% 変数記号，論理記号，量化記号を英語でも何というかを書くべき．
変数記号，論理記号 ($=, \lnot, \lor, \land, \rightarrow$), 量化記号($\forall, \exists$)
と，言語$L$で記述される．
 
% 言語も何と書くか書くべき，
% 述語記号，定数記号，関数記号の集合を区別するべき．
% このあたりはよく検討するべき．というかまあ合わせたほうが良さそう．(富永：なにかしら基礎論の本を参照して書くのはどうでしょうか)
...
% Rを$n$変数述語記号とする．のように書くべき，
% $R \in L$のような書き方を避けるべき．
...
% 定義については引用した方がいいんじゃないか？ということをほのめかされた．
\begin{definition}
     $L$-構造$\M = (M,\dots)$をとる．
...
% 理論$T$が量化記号消去を「持つ」と書くべきか？
% 検討してください．以降の表現も気を付けてください．
...
% 補題番号をローマ数字にしてフォントをかえる，アルファベットに直すとかが適切です．「1.より」とかはだめ．
...
\begin{proof}
     ...
     と$\map{\sigma_i}{\{1, \dots, l_i\}}{P}$を用いて，和積標準形% 「を用いて，表す」は違和感ない．和関標準形は定義していない．
     \begin{equation}
...
% Tarskiの証明はセミナー形式で話しましょう．
% fが多変数だったのに，急に1変数になったりして誤解を招いてるかもしれない？
